 ...
 * token the server no longer honors is exactly what "session expired" means,
 * so registerOnUnauthorized (session.ts) is left to show that notice and
 * redirect. The .catch() below only covers a non-401 failure (a network
 * error), where that handler never fires and we still need to fall back to
 * guest.
 */
