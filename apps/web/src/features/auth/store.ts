...
const TOKEN_STORAGE_KEY = "gasa_company_auth_token";
...
export interface AuthState {
  ...
  token: string | null;
  /** NEVER persisted: roles and company status could go stale, so it's always rehydrated from /auth/me. */
  user: AuthUser | null;
...
/**
 * The single source of truth for company-status routing: RequireActiveCompany
 * and InactivePage both read this, so they can never disagree about which
 * side of /inactive a given user belongs on.
 */
export function selectIsCompanyActive(state: AuthState): boolean {
  return state.user?.company?.status === "active";
}
